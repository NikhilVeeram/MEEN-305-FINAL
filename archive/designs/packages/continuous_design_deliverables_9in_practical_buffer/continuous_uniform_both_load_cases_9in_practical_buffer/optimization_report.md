...
- Total evaluated designs: 744
- Feasible designs found: 223
- Minimum required factor of safety: 1.800
- Objective shaping: penalize excess FoS margin and underutilized stress envelope so the design approaches the chosen FoS target of 1.800 more closely
- Maximum allowed deflection: not enforced in this run
- Random seed: 305

## Practical FoS Buffer Rationale

- This run intentionally used a higher minimum FoS target of 1.800 instead of the project minimum of 1.500.
- Reason: the beam is a thin 3D-printed polymer part, so real print quality, layer bonding, dimensional tolerances, and support contact can reduce the margin relative to the idealized analysis.
- This buffer is a practical fabrication decision, not a change to the project requirement. The project minimum remains 1.5, but the design target was raised to reduce the risk of a part that only barely passes on paper.

## Best Design

- Weight: 0.03396 lbf
- Minimum factor of safety: 1.920
- Excess FoS above target: 0.120
- Governing minimum-FoS load case: Load Case 2 at x = 5.500 in
- Maximum deflection: 0.08919 in
- Uniformity score toward FoS target (0 is best): 0.218972
- Symmetry score (0 is best): 0.000000
- Smoothness score (0 is best): 0.012916
...
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Load Case 1 | upright | 5.000 | 4.000 | 4.500 | 3.740 | 4.500 | 0.03598 | 636.3 |
| Load Case 2 | side | 8.000 | 5.000 | 5.500 | 1.920 | 5.500 | 0.08919 | 1239.4 |
...
- $$b(x)=\begin{cases}0.6195 + (-0.0406)x, & 0 \le x \le 4.5000\\0.4367 + (0.0406)(x-4.5000), & 4.5000 < x \le 9.0000\end{cases}$$
- $$h(x)=\begin{cases}0.4871 + (0.0502)x, & 0 \le x \le 4.5000\\0.7132 + (-0.0502)(x-4.5000), & 4.5000 < x \le 9.0000\end{cases}$$
- $$r(x)=\begin{cases}0.0000 + (0.0000)x, & 0 \le x \le 4.5000\\0.0000 + (0.0000)(x-4.5000), & 4.5000 < x \le 9.0000\end{cases}$$
...
- Search objective used here: minimize weight while enforcing the chosen FoS target of 1.800 and any optional deflection constraints
...
- $W = \rho \int_0^{L_{total}} A(x)\,dx = 0.03396\ \text{lbf}$
...
- $FoS_{min} = \sigma_{allow,eff}/\sigma_{vm,max} = 2380.0/636.3 = 3.740$
- Governing location for this load case: $x = 4.500\ \text{in}$
- $\max |v(x)| = 0.03598\ \text{in}$
- $\max |\theta(x)| = 0.014151\ \text{rad}$
...
- $FoS_{min} = \sigma_{allow,eff}/\sigma_{vm,max} = 2380.0/1239.4 = 1.920$
- Governing location for this load case: $x = 5.500\ \text{in}$
- $\max |v(x)| = 0.08919\ \text{in}$
- $\max |\theta(x)| = 0.035260\ \text{rad}$
