...
- Weight: 0.05003 lbf
- Minimum factor of safety: 2.594
- Excess FoS above target: 1.094
- Governing minimum-FoS load case: Load Case 2 at x = 4.000 in
- Maximum deflection: 0.05061 in
...
- $W = \rho \int_0^L A(x)\,dx = 0.05003\ \text{lbf}$
...
- $\max |v(x)| = 0.04204\ \text{in}$
- $\max |\theta(x)| = 0.014120\ \text{rad}$
...
- $FoS_{min} = \sigma_{allow,eff}/\sigma_{vm,max} = 2380.0/917.5 = 2.594$
- Governing location for this load case: $x = 4.000\ \text{in}$
- $\max |v(x)| = 0.05061\ \text{in}$
- $\max |\theta(x)| = 0.018242\ \text{rad}$
