...
\[
  S(x) = \frac{I(x)}{c(x)},
  \qquad c(x) = \frac{\text{outer vertical dimension}}{2}
\]
...
For the plane-stress state combining bending normal stress and shear:
\[
  \sigma_{vm}(x) = \sqrt{\sigma_b(x)^2 + 3\,\tau(x)^2}
\]

This combines bending and shear into a single scalar comparable to the
effective allowable stress.
...
orientation switches between $I_{upright}$ (LC1) and $I_{side}$ (LC2).
...
$c(x)$ & distance to extreme fiber & in \\
